...
int main(void)
{
  ...
  MX_GPIO_Init();
  MX_TIM3_Init();
  HAL_GPIO_WritePin(GPIOA,GPIO_PIN_1,GPIO_PIN_SET);   // Start the motor in clock wise rotation
  HAL_GPIO_WritePin(GPIOA,GPIO_PIN_2,GPIO_PIN_RESET);


   HAL_TIM_Base_Start(&htim3);               //Initialize stm32 timer 3
   HAL_TIM_PWM_Start(&htim3,TIM_CHANNEL_3);  //PB0 Start pwm motor 100% duty cycle
